...
public class TransformerProcessor {
    ...
    public static void main(String... args) {
        ...
        final ArgumentAcceptingOptionSpec<Path> inputJar = optionParser.accepts("inJar", "Input JAR file to apply transformation to")
                .withRequiredArg()
                .withValuesConvertedBy(new PathConverter(PathProperties.FILE_EXISTING))
                .required();
        final ArgumentAcceptingOptionSpec<Path> atFiles = optionParser.acceptsAll(Arrays.asList("atfile", "atFile"), "Access Transformer File")
                .withRequiredArg()
                .withValuesConvertedBy(new PathConverter(PathProperties.FILE_EXISTING))
                .required();
        ...
        try {
            ...
            if (logFile != null) {
                ...
                Appender fileAppender = FileAppender.newBuilder().setName("logfile").withFileName(logFile).setLayout(configuration.getAppender("SysErr").getLayout()).build();
                fileAppender.start();
    ...
    private static void processJar(final Path inputJar, final Path outputJarPath, final List<Path> atFilePaths) {
        ...
        try (FileSystem outJar = FileSystems.newFileSystem(outJarURI, new HashMap<String, String>() {{
            ...
            try (FileSystem jarFile = FileSystems.newFileSystem(inputJar, ClassLoader.getSystemClassLoader())) {
                Files.walk(StreamSupport.stream(jarFile.getRootDirectories().spliterator(), false)
                        .findFirst()
                        .orElseThrow(() -> new IllegalArgumentException("The JAR has no " + "root?!"))).forEach(path -> {
